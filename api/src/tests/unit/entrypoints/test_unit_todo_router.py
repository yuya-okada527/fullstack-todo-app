...
def test_search_todo_200(client: TestClient, mocker, params):
    mocker.patch("entrypoints.todo_router.search_todo_service", return_value=[])
...
def test_create_todo_200(client: TestClient, mocker, data):
    mocker.patch("entrypoints.todo_router.create_todo_service", return_value=1)
...
def test_create_todo_422(client: TestClient, data):
...
def test_modify_todo_200(client: TestClient, mocker, data):
    mocker.patch("entrypoints.todo_router.modify_todo_service", return_value=1)
    assert client.patch(f"{TODO_API_PATH}/1", json.dumps(data)).status_code == 200, f"data={data} must be valid data"


@pytest.mark.parametrize("data", [
    {"status": "hoge"}
])
def test_modify_todo_422(client: TestClient, data):
    assert client.patch(f"{TODO_API_PATH}/1", json.dumps(data)).status_code == 422, f"data={data} must be invalid data"
